...
#define CORS_HEADERS_TEMPLATE "Access-Control-Allow-Origin: %s\r\nAccess-Control-Allow-Headers:Access-Control-Allow-Headers, Origin,Accept, X-Requested-With, Content-Type, Access-Control-Request-Method, Access-Control-Request-Headers\r\nAccess-Control-Allow-Methods: GET,HEAD,OPTIONS,POST,PUT\r\n"

std::string getCorsHeaders() {
    const char *cors_origin = std::getenv("MO_CORS_ORIGIN");
    if (!cors_origin) {
        cors_origin = "*";  // Default to allow all origins if not specified
    }
    
    char cors_headers[1024];
    snprintf(cors_headers, sizeof(cors_headers), CORS_HEADERS_TEMPLATE, cors_origin);
    return std::string(cors_headers);
}
...
void http_serve(struct mg_connection *c, int ev, void *ev_data) {
    if (ev == MG_EV_ACCEPT) {
        ...
        std::string cors_headers = getCorsHeaders();
        std::string final_headers = std::string(DEFAULT_HEADER) + cors_headers;

        char user[64], pass[64];
        mg_http_creds(message_data, user, sizeof(user), pass, sizeof(pass));
        if (!api_check_basic_auth(user, pass)) {
            mg_http_reply(c, 401, final_headers.c_str(), "Unauthorized. Expect Basic Auth user and / or password\n");
            return;
        }
        ...
        if(mg_match(message_data->uri, mg_str("/api/websocket"), NULL)){
            ...
            mg_http_reply(c, 200, final_headers.c_str(), serialized.c_str());
            return;
            ...
            mg_http_reply(c, status, final_headers.c_str(), resp_buf);
            ...
            mg_http_reply(c, 404, final_headers.c_str(), "API endpoint not found");
        }
    }
}
